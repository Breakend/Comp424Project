...
public class DepthTools {
	...
	public final static int HOP_DEPTH = 1;
...
//		return Math.abs(goal.x-start.x) + Math.abs(goal.y - start.y);
	...
	private static boolean goingBackwards(CCMove move, int playerID){       	
		...
		if(Heuristic.distance(move.getFrom(), p) < Heuristic.distance(move.getTo(), p)){
...
//		return false;
	...
	private static ArrayList<MoveScore> getNextLayerFromHop(CCBoard board, CCMove orighop, CCMove latesthop, int count){
		System.out.println("Hop Layer: " + count);

		ArrayList<MoveScore> list = new ArrayList<MoveScore>();
		for(CCMove move : board.getLegalMoves()){
			System.out.println("HopMove: " + move.toPrettyString());
			if(isReverseMove(move, latesthop) || isMoveBack(orighop, move ) || goingBackwards(move, board.getTurn())) continue;
...
//			System.out.println("GETTING FIRST MOVE POSSIBLE");
		...
		for(CCMove move : board.getLegalMoves()){
			if(isReverseMove(move, lastMove) || goingBackwards(move, playerID)) continue; //goingBackwards(move, board.getTurn()) || 
